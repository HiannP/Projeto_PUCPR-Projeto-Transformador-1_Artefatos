"""
claude.py -- avaliar_modelo_claude.py
================================
Avalia o modelo Claude 3 Haiku na tarefa de classificacao Fake/True
usando o MESMO split 70/15/15 dos treinamentos anteriores.

PRE-REQUISITOS:
    1. pip install anthropic pandas scikit-learn python-dotenv matplotlib
    2. API key configurada no arquivo .env como ANTHROPIC_API_KEY
...
def classificar_uma(client: anthropic.Anthropic, modelo_nome: str, titulo: str, texto: str, tentativas: int = 3) -> int | None:
    ...
    for tentativa in range(tentativas):
        try:
            # Note o uso da API de Messages do Claude
            response = client.messages.create(
                ...
                messages=[
                    {"role": "user", "content": prompt_usuario_local}
                ]
            )
            return parsear_resposta(response.content[0].text)
...
def rodar_predicoes(df: pd.DataFrame, modelo: str, workers: int) -> pd.DataFrame:
    ...
    if not api_key:
        sys.exit(
            "ERRO: defina a variavel de ambiente ANTHROPIC_API_KEY no arquivo .env.\n"
            "Pegue uma chave em: https://console.anthropic.com/"
        )
    ...
    def tarefa(i: int):
        return i, classificar_uma(client, modelo, df.at[i, "title"], df.at[i, "text"])
...
def calcular_e_salvar_metricas(df: pd.DataFrame, pasta_saida: Path, nome_modelo: str) -> None:
    ...
    linhas = [
        "=" * 60,
        f"METRICAS -- {nome_modelo} (Anthropic) no test split",
        "=" * 60,
    ...
    try:
        ...
        ax.set_ylabel("Real")
        ax.set_title(f"Matriz de confusao -- Claude")
        for i in range(2):
            for j in range(2):
        ...
        print("[Aviso] matplotlib não instalado. Matriz de confusão PNG não foi gerada.")


def main():
    p = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument("--amostra-base", type=int, default=5000)
    p.add_argument("--modelo", default="claude-3-haiku-20240307", help="Modelo Anthropic (default: claude-3-haiku-20240307)")
    p.add_argument("--workers", type=int, default=3, help="Workers simultâneos")
    p.add_argument("--saida", default="resultados_claude_haiku_test")
    ...
    print(f"[1/3] Carregando {caminho_csv} e reproduzindo split...")
