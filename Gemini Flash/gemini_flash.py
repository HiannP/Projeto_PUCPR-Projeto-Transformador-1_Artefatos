"""
gemini_flash.py -- avaliar_modelo_gemini.py
================================
Avalia o modelo Gemini (Google) na tarefa de classificacao Fake/True
usando o MESMO split 70/15/15 dos treinamentos anteriores, para permitir
comparacao metodologicamente justa.

PRE-REQUISITOS:
    1. Conta no Google AI Studio: https://aistudio.google.com/
    2. API key configurada no arquivo .env como GEMINI_API_KEY

USO BASICO:
    python gemini_flash.py --csv corpus_fake_news_unificado.csv
"""
...
def classificar_uma(modelo_nome: str, titulo: str, texto: str, tentativas: int = 3) -> int | None:
    ...
    # Configura o modelo com as instrucoes de sistema e formato JSON
    model = genai.GenerativeModel(
        model_name=modelo_nome,
        system_instruction=PROMPT_SISTEMA,
        generation_config=genai.GenerationConfig(
            response_mime_type="application/json",
            temperature=0.0, # Temperatura 0 para respostas deterministicas
    ...
    for tentativa in range(tentativas):
        try:
            ...
            # Tratamento de Rate Limit (429) do Google
            espera = 2 ** tentativa
            ...
            # Algumas vezes a API retorna texto bloqueado por politicas de segurança
...
def main():
    ...
    p.add_argument("--modelo", default="gemini-1.5-flash", help="Modelo Google (default: gemini-1.5-flash)")
    p.add_argument("--workers", type=int, default=3, help="Workers (Google API tem rate limit agressivo na faixa gratuita, ideal 2 ou 3)")
    p.add_argument("--saida", default="resultados_gemini_test")
